...
void PaletteModel::loadPartsAux(QDir & dir, QStringList & nameFilters) {
    ...
    for (int i = 0; i < list.size(); ++i) {
        ...
        // DebugDialog::debug(QObject::tr("part path:%1 core? %2").arg(path).arg(m_loadingCore? "true" : "false"));
        loadPart(path);
    }
...
bool PaletteModel::load(const QString & fileName, ModelBase * refModel, bool doConnections) {
	bool result = ModelBase::load(fileName, refModel, doConnections);
	if (result) {
