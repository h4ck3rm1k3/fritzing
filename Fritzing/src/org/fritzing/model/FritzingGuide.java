 ...
 * FritzingGuide also maintains information about the edge along which those parts are 
 * attached.  This information is useful during resize operations to determine the
 * attachment status of a part.
 * 
 * @author Pratik Shah
 */
public class FritzingGuide 
 ...
 * 							a vertical ruler)
 */
public FritzingGuide(boolean isHorizontal) {
 ...
 */
/**
 * Attaches the given part along the given edge to this guide.  The FritzingSubpart is also
 ...
 */
public void attachPart(FritzingSubpart part, int alignment) {
	if (getMap().containsKey(part) && getAlignment(part) == alignment)
		return;
	
	getMap().put(part, new Integer(alignment));
	FritzingGuide parent = isHorizontal() ? part.getHorizontalGuide() : part.getVerticalGuide();
...
/**
 * Detaches the given part from this guide.  The FritzingSubpart is also updated to reflect
 * this change.
 * 
 * @param	part	the part that is to be detached from this guide
 */
public void detachPart(FritzingSubpart part) {
 ...
 * {@link org.fritzing.edit.FritzingXYLayoutEditPolicy FritzingXYLayoutEditPolicy}
 ...
 * @see		org.fritzing.edit.FritzingXYLayoutEditPolicy#createChangeConstraintCommand(ChangeBoundsRequest, EditPart, Object)
 */
public int getAlignment(FritzingSubpart part) {
 ...
 * 			the keys are FritzingSubparts and values are Integers
 */
public Map getMap() {
	if (map == null) {
		map = new Hashtable();
